...
# on SSU rRNA sequences (for example) and can then be used to extract
# interjacent segments.  We have created HMMs so that each hypervariable
# region (V1-V9) is flanked by two upstream (or left) and two downstream
# (or right) HMMs.  At each position, a long and a short HMM was designed,
# i.e. V1leftlong.HMM and V1leftshort.HMM.  Our HMMs are stored in three
# separate directories for the three taxonomic groups: bacteria, archaea
# and fungi.
...
if (!@INFILES or !defined $OUTFILE or !$result) {
  ...
  Options:
    ...
    Example:
    $0 -a -r .V1-V3. -h HMMs/bacteria/ -o out.fasta  in.fasta
